...
const TongQuan = () => {
  ...
  const fetchCostData = async (year, month = null, quarter = null) => {
    try {
      ...
      if (month) {
        ...
        if (month === 1) {
          responseLastPeriod = await tongQuanService.getMuaHangChartRevenueMonth({
            values: { year: year - 1, month: 12 },
          });
        } else {
          responseLastPeriod = await tongQuanService.getMuaHangChartRevenueMonth({
            values: { year, month: month - 1 },
          });
        }
      } else if (quarter) {
        responseThisPeriod = await tongQuanService.getMuaHangChartRevenueQuarter({
          values: { year, quarter },
        });
        if (quarter === 1) {
          responseLastPeriod = await tongQuanService.getMuaHangChartRevenueQuarter({
            values: { year: year - 1, quarter: 4 },
          });
        } else {
          responseLastPeriod = await tongQuanService.getMuaHangChartRevenueQuarter({
            values: { year, quarter: quarter - 1 },
          });
        }
      } else {
      ...
      const costThisPeriod = responseThisPeriod.data.result.data.map((item) => ({
        cost: item.finalValue,
      }));

      const costLastPeriod = responseLastPeriod.data.result.data.map((item) => ({
        cost: item.finalValue,
      }));

      setCostData({
        thisPeriod: costThisPeriod,
        lastPeriod: costLastPeriod,
      });
    } catch (error) {
              ...
              <Legend
                onClick={(e) => {
                  if (e.dataKey === "Doanh thu") {
                    setHideRevenue(!hideRevenue); // Toggle trạng thái của "Doanh thu"
                  } else if (e.dataKey === "Chi phí kỳ này") {
                    setHideCostThisPeriod(!hideCostThisPeriod); // Toggle trạng thái của "Chi phí kỳ này"
                  } else if (e.dataKey === "Chi phí kỳ trước") {
                    setHideCostLastPeriod(!hideCostLastPeriod); // Toggle trạng thái của "Chi phí kỳ trước"
                  }
                }}
              />
              <Line
                dataKey="Doanh thu"
                stroke="#4B8AF1"
                strokeOpacity={hideRevenue ? 0 : 1} // Ẩn hoặc hiện dựa trên trạng thái
                activeDot={{ r: 8 }}
              />
              <Line
                dataKey="Chi phí kỳ này"
                stroke="#E76F51"
                strokeOpacity={hideCostThisPeriod ? 0 : 1}
              />
              <Line
                dataKey="Chi phí kỳ trước"
                stroke="#82ca9d"
